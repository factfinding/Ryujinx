...
namespace ARMeilleure.CodeGen.LoongArch64
{
    class CodeGenContext
    {
        ...
        private readonly Dictionary<BasicBlock, List<(LoongArch64Condition Condition, long BranchPos)>> _pendingBranches;
        ...
        private LoongArch64Condition _jNearCondition;
        ...
        public CodeGenContext(AllocationResult allocResult, int maxCallArgs, bool relocatable)
        {
            ...
            _pendingBranches = new Dictionary<BasicBlock, List<(LoongArch64Condition, long)>>();
        ...
        public void EnterBlock(BasicBlock block)
        {
            ...
            if (_pendingBranches.TryGetValue(block, out List<(LoongArch64Condition Condition, long BranchPos)> list))
            {
                foreach ((LoongArch64Condition condition, long branchPos) in list)
                {
        ...
        {
            JumpTo(LoongArch64Condition.Al, target);
        }

        public void JumpTo(LoongArch64Condition condition, BasicBlock target)
        {
            ...
            {
                if (!_pendingBranches.TryGetValue(target, out List<(LoongArch64Condition Condition, long BranchPos)> list))
                {
                    list = new List<(LoongArch64Condition, long)>();
        ...
        private void WriteBranch(LoongArch64Condition condition, long to)
        {
            int imm = checked((int)(to - _stream.Position));

            if (condition != LoongArch64Condition.Al)
            {
        ...
        public void JumpToNear(LoongArch64Condition condition)
        {
